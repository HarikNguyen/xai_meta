...
def main():
    ...
    weights = [
        p.clone().to("cuda").detach().requires_grad_(True) for p in model.parameters()
    ]
    ...
    torch.save(weights, "weights/weights_final.pt")
...
def test(model, weights, loader_iter):
    ...
    avg_pred = (
        sum(p[0] for p in preds) / len(preds),
        sum(p[1] for p in preds) / len(preds),
    )
    avg_post = (
        sum(p[0] for p in posts) / len(posts),
        sum(p[1] for p in posts) / len(posts),
    )
