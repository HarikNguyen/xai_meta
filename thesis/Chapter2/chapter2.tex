...
\begin{equation}
    \centering
    \phi_j = \phi_{j-1} - \alpha \nabla_{\phi} \mathcal{L}_{S_i}(\phi_{j-1}), \qquad j = 1, 2, \dots, T, \quad \phi_{0} = \theta_0
    \label{eq:inner_loop}
\end{equation}
...
\subsection{Các phương pháp hậu diễn giải dựa trên gradients và nhiễu loạn}
...
\textbf{Hướng thứ hai} tích hợp trực tiếp các kỹ thuật dựa trên CAM hoặc các cơ chế diễn giải nội tại vào quy trình huấn luyện/suy luận few-shot nhằm giám sát mô hình tập trung vào đặc trưng ngữ nghĩa thay vì tương quan giả. Một số framework như \textit{expert-guided} cho ảnh y khoa \cite{uddin2025expert} là đại diện sử dụng hướng đi này, bằng cách sử dụng Grad-CAM kết hợp với hàm mất mát căn chỉnh (alignment loss) giữa bản đồ nhiệt (heat map) và vùng chú thích đã được chuyên gia tạo sẵn (ground-truth). Tương tự, MTUNet \cite{wang2021mtunet} thiết kế một bộ so khớp (matcher) so sánh trực tiếp các patch đặc trưng giữa ảnh hỗ trợ và ảnh truy vấn, dùng điểm số so khớp làm căn cứ trực quan cho quyết định phân loại. Do đó, điểm hạn chế chung của các phương pháp này là bản đồ nhiệt (hoặc điểm so khớp) chỉ được trích xuất tại thời điểm mô hình \textit{đã thích ứng hoàn tất} ($f_{\phi_i^*}$) tức là chỉ diễn giải kết quả suy luận cuối cùng, và một số còn cần các vùng chú thích được tạo sẵn hoặc phải thiết kế lại kiến trúc (intrinsic). Thành thử, các phương pháp này hoàn toàn bỏ qua việc giải thích bản chất của quá trình dịch chuyển tham số từ điểm khởi tạo $\theta_0$ sang trạng thái thích ứng $\phi_i^*$.
...
Mặt khác, khái niệm quỹ đạo tham số trong bối cảnh gradient-based meta-learning cũng đã được khai thác bởi Li và cộng sự thông qua A-MAML \cite{li2023adjointmaml}, phương pháp xem quá trình gradient descent của inner-loop như một phương trình vi phân thường (ODE) và dùng phương pháp liên hợp (adjoint method) để tính hiệu quả gradient của $\theta_0$ mà không cần lưu trữ đồ thị tính toán trung gian. Tuy nhiên, mục tiêu của A-MAML hoàn toàn khác với hướng tiếp cận của đề tài này: đây là một kỹ thuật nhằm \emph{tối ưu hóa hiệu quả tính toán lúc meta-training}, không phải một phương pháp \emph{hậu diễn giải} (post-hoc) cho một mô hình đã huấn luyện xong. Điểm tương đồng duy nhất giữa hai hướng là cùng sử dụng công cụ toán học liên hợp (adjoint/Pearlmutter HVP), trong khi bài toán được giải quyết là hoàn toàn khác biệt.
