...
flags.DEFINE_integer(
    "metatrain_iterations", 15000, "number of metatraining iterations."
...
flags.DEFINE_float(
    "update_lr", 1e-3, "step size alpha for inner gradient update."
)  # 0.1 for omniglot
flags.DEFINE_integer(
...
def train(model, saver, sess, exp_string, data_generator, resume_itr=0):
    ...
    for itr in range(
        resume_itr, FLAGS.pretrain_iterations + FLAGS.metatrain_iterations
    ):
        feed_dict = {}
        if "generate" in dir(data_generator):
            ...
            inputb = batch_x[
                :, num_classes * FLAGS.update_batch_size :, :
            ]  # b used for testing
        ...
        if (
            (itr != 0)
            and itr % TEST_PRINT_INTERVAL == 0
            and FLAGS.datasource != "sinusoid"
        ):
            if "generate" not in dir(data_generator):
                ...
                if model.classification:
                    ...
                    input_tensors = [
                        ...
                        model.metaval_total_losses2[FLAGS.num_updates - 1],
                        model.summ_op,
                    ]
            else:
...
def main():
    ...
    if FLAGS.datasource == "sinusoid":
        ...
        if FLAGS.metatrain_iterations == 0 and FLAGS.datasource == "miniimagenet":
            ...
            data_generator = DataGenerator(
                1, FLAGS.meta_batch_size
            )  # only use one datapoint,
        else:
            if (
    ...
    exp_string = (
        ...
        + ".mbs_"
        + str(FLAGS.meta_batch_size)
        + ".ubs_"
        + str(FLAGS.train_update_batch_size)
        + ".numstep"
        + str(FLAGS.num_updates)
        + ".updatelr"
        + str(FLAGS.train_update_lr)
    )
    ...
    if FLAGS.resume or not FLAGS.train:
        ...
        if FLAGS.test_iter > 0:
            model_file = (
                model_file[: model_file.index("model")] + "model" + str(FLAGS.test_iter)
            )
        if model_file:
